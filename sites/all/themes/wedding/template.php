...
function wedding_form_alter(&$form, &$form_state, $form_id) {
  ...
  if ($form_id == 'search_block_form') {
    ...
    $form['search_block_form']['#attributes']['onblur'] = "if (this.value == '') {this.value = 'Search'; this.style.color='#ccc';}";
    $form['search_block_form']['#attributes']['onfocus'] = "if (this.value == 'Search') {this.value = ''; this.style.color='#000';}";
  }
}

function wedding_form_views_exposed_form_alter(&$form, $form_state) {
  // Resort the dropdown boxes so they appear in alphabetical order.
  ...
  // Now we need replace the values in the City field to a grouped dropdown box.
  $field_id = 'field_city';
...
function wedding_textarea($variables) {
